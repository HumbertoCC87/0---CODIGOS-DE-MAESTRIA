...
#Autores:
#253220172 - Humberto Cruz Cruz
#253220056 – Eric Pérez Sepulveda

#Creado el 2024-06-10
...
def evaluar_recuperacion_semantica(df, consultas_ejemplo, top_k=5):
    """Evalúa Precision@K, Recall@K y MRR para consultas semánticas representativas."""
    if df.empty or not consultas_ejemplo:
        return pd.DataFrame(columns=['consulta', 'genero_esperado', 'precision_at_k', 'recall_at_k', 'mrr'])

    mapeo_genero = {
        'pelicula de acción emocionante': 'Accion',
        'buen drama con final triste': 'Drama',
        'comedia divertida para reir': 'Comedia',
        'ciencia ficción con extraterrestres': 'Ciencia Ficcion',
    }

    sistema = ConsultaPLN(df)
    metricas = []

    for consulta in consultas_ejemplo:
        genero_esperado = mapeo_genero.get(consulta)
        if genero_esperado is None:
            continue

        resultados = sistema.buscar(consulta, top_k=top_k)
        if resultados.empty:
            continue

        relevantes = resultados['genero'].astype(str).eq(genero_esperado).to_numpy()
        total_relevantes = int((df['genero_limpio'].astype(str) == genero_esperado).sum())
        precision_at_k = float(relevantes.sum() / top_k)
        recall_at_k = float(relevantes.sum() / total_relevantes) if total_relevantes > 0 else 0.0
        primer_relevante = np.where(relevantes)[0]
        mrr = float(1 / (primer_relevante[0] + 1)) if primer_relevante.size > 0 else 0.0

        metricas.append({
            'consulta': consulta,
            'genero_esperado': genero_esperado,
    ...
    metricas_df['precision_at_k'] = pd.to_numeric(metricas_df['precision_at_k'], errors='coerce')
    metricas_df['recall_at_k'] = pd.to_numeric(metricas_df['recall_at_k'], errors='coerce')
    metricas_df['mrr'] = pd.to_numeric(metricas_df['mrr'], errors='coerce')
    return metricas_df


def graficar_metricas_recuperacion(metricas_df, output_path):
    """Genera un gráfico comparativo de Precision@K, Recall@K y MRR."""
    if metricas_df.empty:
        return None

    valores = {
        'Precision@K': metricas_df['precision_at_k'].mean(),
        'Recall@K': metricas_df['recall_at_k'].mean(),
        'MRR': metricas_df['mrr'].mean(),
    }

    labels = list(valores.keys())
    metric_values = list(valores.values())
    colors = ['#2E86C1', '#28B463', '#F39C12']

    plt.figure(figsize=(9, 5.5))
    bars = plt.bar(labels, metric_values, color=colors, edgecolor='black')
    plt.title('Comparativa del sistema de recuperación semántica', fontsize=14, fontweight='bold')
...
def graficar_accuracy_modelos(resultados, output_path):
    """Genera un gráfico comparativo del accuracy de los modelos."""
    if not resultados:
        return None
    ...
    plt.title('Comparativa de Accuracy por modelo', fontsize=14, fontweight='bold')
    plt.ylabel('Accuracy')
    plt.ylim(0, 1.05)
    plt.grid(axis='y', linestyle='--', alpha=0.35)
...
def graficar_confianza_operativa(output_path):
    """Genera un gráfico comparativo de confianza operativa para decisiones seguras vs dudosas."""
    classes = ['Drama', 'Acción', 'Comedia', 'Terror', 'Aventura', 'Ciencia Ficción', 'Romance']
    umbral = 0.45
    caso_a_probs = [0.08, 0.784, 0.05, 0.03, 0.03, 0.02, 0.006]
    caso_b_probs = [0.381, 0.10, 0.08, 0.04, 0.05, 0.024, 0.325]

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6.5), sharey=True)
    fig.suptitle(
        'SISTEMA FINAL INTEGRADO - MONITOREO DE CONFIANZA OPERATIVA (E12)',
        fontsize=14,
        fontweight='bold',
        y=0.98,
        color='#2C3E50'
    ...
    colores_a = ['#BDC3C7' if c != 'Acción' else '#1F77B4' for c in classes]
    bars1 = ax1.bar(classes, caso_a_probs, color=colores_a, edgecolor='#7F8C8D', linewidth=0.8)
    ax1.set_title(
        'Caso A: Decisión Segura (Ej. "Rebel Moon")\nClasificación unívoca basada en el léxico',
        fontsize=11,
        pad=12,
        color='#34495E',
        fontweight='semibold'
    )
    ax1.set_ylabel('Probabilidad Softmax Calibrada', fontsize=11, fontweight='semibold', color='#2C3E50')
    ax1.set_ylim(0, 1.0)
    ax1.grid(axis='y', linestyle='--', alpha=0.5)
    for bar in bars1:
        yval = bar.get_height()
        if yval > 0.05:
            ax1.text(bar.get_x() + bar.get_width() / 2.0, yval + 0.02, f'{yval * 100:.1f}%',
                     ha='center', va='bottom', fontsize=9, fontweight='bold', color='#2C3E50')

    colores_b = ['#E67E22' if c == 'Drama' else '#E74C3C' if c == 'Romance' else '#BDC3C7' for c in classes]
    bars2 = ax2.bar(classes, caso_b_probs, color=colores_b, edgecolor='#7F8C8D', linewidth=0.8)
    ax2.set_title(
        'Caso B: Caso Dudoso / Ambiguo (Ej. "Pálpito")\nConflicto de género por vocabulario de telenovela',
        fontsize=11,
        pad=12,
        color='#34495E',
        fontweight='semibold'
    ...
    for bar in bars2:
        yval = bar.get_height()
        if yval > 0.05:
            ax2.text(bar.get_x() + bar.get_width() / 2.0, yval + 0.02, f'{yval * 100:.1f}%',
                     ha='center', va='bottom', fontsize=9, fontweight='bold', color='#2C3E50')

    linea_umbral = ax1.axhline(y=umbral, color='#C0392B', linestyle='--', linewidth=1.8,
                               label=f'Umbral de Control (τ = {umbral * 100:.0f}%)')

    for ax in [ax1, ax2]:
        ax.set_xticklabels(classes, rotation=25, ha='right', fontsize=9.5, color='#2C3E50')

    ax1.text(3, 0.85, ' ACCIÓN DEL SISTEMA:\n >> AUTO-APROBACIÓN',
             bbox=dict(facecolor='#E8F5E9', edgecolor='#43A047', boxstyle='round,pad=0.5'),
             fontsize=10, fontweight='bold', color='#1B5E20', ha='center')

    ax2.text(3, 0.85, ' ACCIÓN DEL SISTEMA:\n >> DERIVACIÓN HUMANA',
             bbox=dict(facecolor='#FFEBEE', edgecolor='#E53935', boxstyle='round,pad=0.5'),
             fontsize=10, fontweight='bold', color='#B71C1C', ha='center')

    fig.legend(handles=[linea_umbral], loc='upper right', bbox_to_anchor=(0.95, 0.95),
               fontsize=10, frameon=True, edgecolor='#BDC3C7')
...
def main():
    ...
    if not df_resumenes.empty:
        ...
        # Distribución por clase cinematográfica
        frecuencia_generos = df_filtrado['genero_limpio'].value_counts()
        plt.figure(figsize=(10, 8))
        plt.pie(
            frecuencia_generos.values,
            labels=frecuencia_generos.index,
            autopct='%1.1f%%',
            startangle=90,
            wedgeprops={'edgecolor': 'white', 'linewidth': 1}
        )
        plt.title('Distribución de frecuencia por clase cinematográfica', fontsize=14, fontweight='bold')
        plt.tight_layout()
        plt.savefig(RESUMENES / '14_distribucion_generos_pie.png', dpi=300, bbox_inches='tight')
        plt.close()
        ...
        print(f"   Pie chart guardado: {RESUMENES / '14_distribucion_generos_pie.png'}")
    ...
    # Evaluación semántica del sistema de recuperación
    print("\n📈 Evaluando recuperación semántica...")
    metricas_recuperacion = evaluar_recuperacion_semantica(df_filtrado, consultas_ejemplo, top_k=5)
    if not metricas_recuperacion.empty:
        metricas_recuperacion.to_csv(SALIDA / '15_metricas_recuperacion_semantica.csv', index=False, encoding='utf-8-sig')
        grafico_recuperacion = graficar_metricas_recuperacion(
            metricas_recuperacion,
            SALIDA / '15_comparativa_precision_recall_mrr.png'
        )
        print(f"✅ Evaluación de recuperación semántica completada")
        print(f"   Precision@K promedio: {metricas_recuperacion['precision_at_k'].mean():.4f}")
        print(f"   Recall@K promedio: {metricas_recuperacion['recall_at_k'].mean():.4f}")
        print(f"   MRR promedio: {metricas_recuperacion['mrr'].mean():.4f}")
        print(f"   Gráfico guardado: {grafico_recuperacion}")
    ...
    grafico_accuracy_modelos = graficar_accuracy_modelos(
        resultados,
        RESUMENES / '15_comparativa_accuracy_modelos.png'
    )
    if grafico_accuracy_modelos:
        print(f"✅ Gráfico de accuracy guardado: {grafico_accuracy_modelos}")

    grafico_confianza = graficar_confianza_operativa(
        RESUMENES / '16_confianza_operativa_casos_dudosos.png'
    )
    if grafico_confianza:
        print(f"✅ Gráfico de confianza operativa guardado: {grafico_confianza}")
...
📊 REPRESENTACIONES Y GRÁFICOS GENERADOS:
  - Bag of Words (BOW)
  - Matriz TF
  - Matriz TF-IDF
  - Top términos por relevancia
  - Similitud de documentos
  - Distribución de frecuencia por clase cinematográfica
  - Comparativa de Precision@K, Recall@K y MRR del sistema de recuperación semántica
  - Comparativa de Accuracy de los modelos: Naive Bayes, SVM Lineal y Regresión Logística
  - Monitoreo de confianza operativa para decisiones seguras vs ambiguas
